...
export function loadIllustrationRules(rulesPath = DEFAULT_RULES_PATH) {
  const out = { excludeTypes: [], excludeTags: [] };
  try {
    const content = fs.readFileSync(rulesPath, 'utf8');
    const yamlMatch = content.match(/```yaml\s*([\s\S]*?)```/);
    if (!yamlMatch) return out;
    ...
    for (const line of lines) {
      const keyMatch = line.match(/^exclude_types:\s*$/);
      if (keyMatch) {
        currentKey = 'excludeTypes';
        continue;
      }
      const keyMatch2 = line.match(/^exclude_tags:\s*$/);
      if (keyMatch2) {
      ...
      if (listItem && currentKey) {
        ...
        if (currentKey === 'excludeTypes') out.excludeTypes.push(val);
        else out.excludeTags.push(val);
      }
    }
  } catch (err) {
    if (err.code !== 'ENOENT') {
