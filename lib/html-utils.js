...
export function insertFigures(blocks, illustrations) {
  ...
  for (const item of withOrder) {
    const rawIdx = item.insertAfterBlockIndex ?? 0;
    const idx = rawIdx + 1;
    if (idx < 0 || idx > result.length) {
      console.warn(`[insertFigures] insertAfterBlockIndex ${rawIdx} 超出範圍（共 ${result.length} 個區塊），改為附加至末尾`);
...
export function buildFigcaptionInner(item) {
  ...
  const pUrl = isSafeUrl(photographerUrl) ? escapeHtmlAttr(photographerUrl) : '#';
  const sUrl = isSafeUrl(sourceUrl) ? escapeHtmlAttr(sourceUrl) : 'https://www.pexels.com';
...
/**
 * 檢查 URL 是否為安全的 http/https 協議，防止 javascript: 等協議注入。
 * @param {string} url
 * @returns {boolean}
 */
