...
const config = {
  ...
  /** Pexels API 金鑰（使用圖庫來源時） */
  PEXELS_API_KEY: process.env.PEXELS_API_KEY ?? '',
  /** Pexels 插圖是否在 figcaption 顯示攝影師／來源（預設開啟） */
  PEXELS_ATTRIBUTION_DEFAULT: process.env.PEXELS_ATTRIBUTION_DEFAULT !== 'false',
  /** 為 true 時任一幅插圖失敗即中斷（等同舊版行為） */
  ILLUSTRATION_STRICT_MODE: process.env.ILLUSTRATION_STRICT_MODE === 'true',
};
